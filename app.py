import pandas as pd
from flask import Flask, Response, flash, make_response, request, redirect, send_from_directory, url_for, render_template
...
'''@app.route("/myhealthc")
def myhealthc():
    # Get the absolute path of the web_app folder
    web_app_dir = os.path.abspath("Covid-19")
    
    # Serve the file
    return send_from_directory(web_app_dir, "covidindex.html")

    # Serve the CSS files
    css_files = [
        send_from_directory(web_app_dir, "style.css")
        # Add more CSS files here as needed
    ]

    # Return the HTML file with the CSS files included
    return Response(html_file, mimetype="text/html", headers={"Cache-Control": "no-cache"}), css_files'''
...
def resultc():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            return render_template('resultc.html', filename=filename, fn=firstname, ln=lastname, age=age, r=pred, gender=gender, em=email, ph=phone)
...
def resultbt():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            return render_template('resultbt.html', filename=filename, fn=firstname, ln=lastname, age=age, r=pred, gender=gender, em=email, ph=phone)
...
def resultd():
    if request.method == 'POST':
        ...
        return render_template('resultd.html', fn=firstname, ln=lastname, age=age, r=pred, gender=gender, p=pregnancies,bmi=bmi,
                               gl=glucose,bp=bloodpressure,i=insulin, dp=diabetespedigree, sk=skinthickness, em=email, ph=phone)
...
def resultbc():
    if request.method == 'POST':
        ...
        return render_template('resultbc.html', fn=firstname, ln=lastname, age=age, r=pred, gender=gender, em=email, ph=phone)
...
def resulta():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            return render_template('resulta.html', filename=filename, fn=firstname, ln=lastname, age=age, r=pred, gender=gender, em=email, ph=phone)
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            if pred < 0.5:
                ...
                pred = 1
            # pb.push_sms(pb.devices[0],str(phone), 'Hello {},\nYour COVID-19 test results are ready.\nRESULT: {}'.format(firstname,['POSITIVE','NEGATIVE'][pred]))
            return render_template('resultp.html', filename=filename, fn=firstname, ln=lastname, age=age, r=pred, gender=gender, em=email, ph=phone)
...
def resulth():
    if request.method == 'POST':
        ...
        # pb.push_sms(pb.devices[0],str(phone), 'Hello {},\nYour Diabetes test results are ready.\nRESULT: {}'.format(firstname,['NEGATIVE','POSITIVE'][pred]))
        return render_template('resulth.html', fn=firstname, ln=lastname, age=age, r=pred, gender=gender, nv=nmv, t=thal,mhr=mhra, 
                               tc=tcp, ei=eia, o=op, em=email, ph=phone)
